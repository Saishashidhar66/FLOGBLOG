...
def write_blog():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO blog(title,body,author) VALUES(%s, %s,%s)", (title, author))
        mysql.connection.commit()
        cur.close()
...
def view_blogs():
    ...
    if result_value > 0:
        ...
        return render_template('my_blog.html',my_blogs=my_blogs)
    else:
        return render_template('my_blog.html',my_blogs=None)
